...
export async function adminUploadFiles(
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/v1/upload?path=${encodeURIComponent(path)}`,
      {
        method: "POST",
        headers: {
...
export async function createDirectory(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/v1/addfile?dirname=${encodeURIComponent(dirname)}&path=${encodeURIComponent(path)}`,
      {
        method: "POST",
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Failed to create directory with status ${response.status}`,
      );
    }
...
export async function getFiles(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/v1/filelist?path=${encodeURIComponent(path)}&page=${page}&limit=${limit}`,
      {
        method: "GET",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function deleteFile(
  filename: string,
  path: string = "/",
): Promise<{ success: boolean; isDir?: boolean }> {
  try {
    const token = localStorage.getItem("token");
    if (!token) {
      throw new Error("No authentication token found");
    ...
    const response = await fetch(
      `${API_BASE_URL}/v1/delete?filename=${encodeURIComponent(filename)}&path=${encodeURIComponent(path)}`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`,
...
export async function renameFile(
  ...
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/v1/rename?old_filename=${encodeURIComponent(oldFilename)}&new_filename=${encodeURIComponent(newFilename)}&path=${encodeURIComponent(path)}`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${token}`,
